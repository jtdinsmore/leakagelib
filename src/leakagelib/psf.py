...
class PSF:
    ...
    def _anti_rotate(image, rotation_deg):
        '''
    ...
    def sky_cal(detector, source, rotation, psf_origin="merge-nn", clip=True):
        ...
        ----------
        detector : {1, 2, 3}
            Detector number: 1 for DU1, 2 for DU2, 3 for DU3.
        source : Source
            Source object used to set the width of the PSF.
        rotation : float
            Angle in radians to rotate the PSF into the detector's frame.
        psf_origin : str
            Name of the sky-calibrated PSF to use.
    ...
    def ground_cal(detector, source, rotation, ground_blur=GROUND_BLUR, clip=True):
        ...
        Parameters
        ----------
        detector_index : {1, 2, 3}
            Detector number: 1 for DU1, 2 for DU2, 3 for DU3.
        source : Source
            Source object used to set the width of the PSF.
        rotation : float
            Angle in radians to rotate the PSF into the detector's frame.
        ground_blur : float, optional
            Amount to blur the ground PSFs. Default is a manually tuned value to match leakage patterns.
    ...
    def __init__(self, image, current_pixel_width, blur_width, source, rotation, detector, clip=False):
        ...
        self.psf = image
        if rotation != 0:
            self.psf = PSF._rotate(self.psf, rotation * 180 / np.pi)
    ...
    def blur_custom_kernel(self, kernel):
        ...
        ----------
        kernel: array-like
            The kernel should be a 2d array with pixels assumed to be the same size as this PSF's pixels. The width of the sky calibrated pixels is 2.9729
        """
        # Blur
    ...
    def save(self, directory, header=None):
        ...
        hdul.writeto(f"{directory}/PSF_MMA{self.det}.fits", overwrite=True)