...
def fit_extended(source, psfs, spectrum, is_obs, qs_obs, us_obs, initial_source_pol=None, inertia=None, num_iter=5000, max_rate=1e-2, report_frequency=50, regularize_coeff=1, energy_dependence=None):
    ...
    ----------
    source : Source
        The Source object containing the resolved I flux. Any source polarization
        stored in the object is ignored.
    psf : list of PSF
        List of PSFs for each detector.
    spectrum : Spectrum
        The Spectrum object of the source.
    is_obs : list of ndarray
        Observed I maps for each detector.
    qs_obs : list of ndarray
        Observed q (normalized) maps for each detector.
    us_obs : list of ndarray
        Observed u (normalized) maps for each detector.
    initial_source_pol : ndarray, optional
        Array to initialize the gradient descent method. If None, the observed
        polarization in DU1 is used.
    inertia : ndarray, optional
        Inertia map for gradient descent. High inertia reduces the step size in a pixel.
        If None, inertia is set to the square root of the number of counts.
    num_iter : int
        Number of iterations to perform.
    max_rate : float
        Maximum gradient descent rate at the beginning.
    report_frequency : int or None, optional
        Frequency at which to save a snapshot of the algorithm's state. None for no snapshots.
    regularize_coeff : float, optional
        Strength of the regularization term. Zero disables regularization.

    Returns
    -------
    tuple of (ndarray, ndarray, matplotlib.animation.Animation or None)
        Tuple containing `(q_map, u_map, animation)`. If `report_frequency` was None,
        `animation` is None.
    """
