...
def fit(energy_cut=(2,8)):
    ...
    # Add a particle background component. The spectrum is automatically set if you set other components' spectra
    settings.add_particle_source()
    settings.fix_qu("pbkg", (0,0))
    ...
    settings.set_lightcurve("src", lambda t: 1 + np.sin(t)) # Oscillating ligthcurve

    # You can even add polarization sweep models
    settings.set_sweep("src", (lambda t: np.sin(2*t), lambda t: np.cos(2*t))) # Model the PA as rotating with frequency 1 Hz, so that q and u (provided) oscillate with frequency 2 Hz.
